...
const schema = z.object({
  ...
  SEARCH_PROVIDER: z.enum(['groq', 'xai']).default('groq'),
  GROQ_API_KEY: z.string().trim().optional(),
  GROQ_MODEL: z.string().trim().default('groq/compound'),
  GROQ_DAILY_SEARCH_BUDGET: z.coerce.number().int().positive().max(250).default(20),
